...
def init_filled_particles(pos, shs, cov, opacity, new_pos):
    ...
    # ti_opacity.from_torch(opacity.reshape(-1))
    ti_pos.from_torch(pos.detach().contiguous().reshape(-1, 3))
    ti_cov.from_torch(cov.detach().contiguous().reshape(-1, 6))
    ti_shs.from_torch(shs.detach().contiguous())
    ti_opacity.from_torch(opacity.detach().contiguous().reshape(-1))
...
def init_filled_particles2(pos, shs, rot, scale, opacity, new_pos):
    ...
    ti_pos.from_torch(pos.detach().contiguous().reshape(-1, 3))
    ti_scale.from_torch(scale.detach().contiguous().reshape(-1, 3))
    ti_shs.from_torch(shs.detach().contiguous())
    ti_opacity.from_torch(opacity.detach().contiguous().reshape(-1))
    ti_rot.from_torch(rot.detach().contiguous().reshape(-1, 4))

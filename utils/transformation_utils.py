...
def get_mat_from_upper(upper_mat, device="cuda"):
    upper_mat = upper_mat.reshape(-1, 6)
    mat = torch.zeros((upper_mat.shape[0], 9), device=device)
    mat[:, :3] = upper_mat[:, :3]
    mat[:, 3] = upper_mat[:, 1]
    mat[:, 4] = upper_mat[:, 3]
    mat[:, 5] = upper_mat[:, 4]
...
def get_uppder_from_mat(mat, device="cuda"):
    mat = mat.view(-1, 9)
    upper_mat = torch.zeros((mat.shape[0], 6), device=device)
    upper_mat[:, :3] = mat[:, :3]
    upper_mat[:, 3] = mat[:, 4]
    upper_mat[:, 4] = mat[:, 5]
    upper_mat[:, 5] = mat[:, 8]
...
def apply_inverse_cov_rotations(upper_cov_tensor, rotation_matrices, device="cuda"):
    cov_tensor = get_mat_from_upper(upper_cov_tensor, device=device)
    for i in range(len(rotation_matrices)):
        R = rotation_matrices[len(rotation_matrices) - 1 - i]
        cov_tensor = apply_cov_rotation(cov_tensor, R.T)
    return get_uppder_from_mat(cov_tensor, device=device)
